...
module TrisanoHelper
  ...
  def wait_for_element_present(name, browser=@browser)
    wait_for_condition do
      browser.is_element_present(name)
    end
  end

  def wait_for_element_not_present(name, browser=@browser)
    wait_for_condition do
      not browser.is_element_present(name)
    end
  end

  def wait_for_xpath_present(xpath, browser=@browser)
    wait_for_condition do
      not browser.get_by_xpath(xpath).nil?
    end
  end

  def wait_for_link_present(link_text, browser=@browser)
    wait_for_condition do
      not browser.anchor_by_text(link_text).nil?
    end
  end

  def wait_for_condition(browser=@browser)
    20.times do
      break if yield rescue false
      browser.current_page.synchronized do
        browser.current_page.wait(500)
      end
    end
  end
